...
export function FilterFor(inputKey?: string): MethodDecorator {
  return (target, propertyKey) => {
    ...
    const existing = (Reflect.getOwnMetadata(FILTER_FOR_METADATA, ctor) ?? new Map<string, string>()) as Map<string, string>;
...
export function getFilterForMap(target: object): Map<string, string> {
  const own = Reflect.getOwnMetadata(FILTER_FOR_METADATA, target) as Map<string, string> | undefined;
