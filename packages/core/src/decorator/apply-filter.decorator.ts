...
export function ApplyFilter(filterClass: Type<unknown>, options: ApplyFilterOptions = {}): ParameterDecorator {
  return (target, propertyKey, parameterIndex) => {
    const existing = (Reflect.getOwnMetadata(APPLY_FILTER_METADATA, target.constructor, propertyKey as string) ??
      []) as ApplyFilterMetadataEntry[];
    existing.push({ filterClass, options, paramIndex: parameterIndex });
    Reflect.defineMetadata(APPLY_FILTER_METADATA, existing, target.constructor, propertyKey as string);
...
export function getApplyFilterMetadata(
  ...
  return (Reflect.getOwnMetadata(APPLY_FILTER_METADATA, controllerCtor, methodName as string) ?? []) as ApplyFilterMetadataEntry[];
}
