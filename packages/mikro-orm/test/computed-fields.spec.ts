...
// A function-source computed field: a correlated subquery counting each
// author's books, built from the real build-time alias handed to the
// callback.
@Injectable()
@Filterable({
  entity: Author,
...
describe('MikroORM computed / virtual fields', () => {
  ...
  async function createModule() {
    const mod = await Test.createTestingModule({
      imports: [
        ...
        FilterModule.forFeature([AuthorFilterFn, AuthorFilterQb, AuthorFilterVerbatim]),
      ],
    }).compile();
  ...
  it('computed sort composes with a real-column sort, preserving order', async () => {
    ...
    await runner.apply(AuthorFilterFn, { filter: {}, sort: '-booksCount,name' }, qb);
  ...
  it('parameterizes the client value (no SQL injection via the value)', async () => {
    ...
    await seed();
    const qb = orm.em.fork().createQueryBuilder(Author);
    await runner.apply(AuthorFilterFn, { filter: { booksCount: { equals: 0 } } }, qb);
