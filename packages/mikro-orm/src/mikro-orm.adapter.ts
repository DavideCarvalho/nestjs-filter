...
export class MikroOrmAdapter implements FilterAdapter {
  ...
  /**
   * Wraps a dev-declared computed expression as a MikroORM raw fragment usable
   * as an ORDER BY / WHERE key. A `{alias}` token is substituted at query-build
   * time with the root entity alias MikroORM assigned (autogenerated, e.g.
   * `a0`), which lets a correlated subquery reference the outer row
   * (`(SELECT COUNT(*) FROM child WHERE child.parent_id = {alias}.id)`). A fresh
   * fragment is minted per call: MikroORM consumes a raw fragment on first use,
   * so the same object cannot be reused across two conditions.
   */
  private computedRaw(expression: string) {
    return expression.includes('{alias}')
      ? raw((alias) => expression.replaceAll('{alias}', alias))
      : raw(expression);
  }

  applyComputedSort(qb: unknown, expression: string, direction: 'asc' | 'desc'): void {
    const queryBuilder = qb as { andOrderBy: (order: Record<string, unknown>) => void };
    queryBuilder.andOrderBy({ [this.computedRaw(expression)]: direction });
  }

  applyComputedField(qb: unknown, expression: string, value: unknown): void {
    // The dev expression is inlined as the raw left-hand side; the client value
    ...
    // fresh raw fragment per operator (see `computedRaw`) avoids reuse.
    const conditions = valueToColumnFilters('__computed__', value).map((filter) =>
      resolveOperator({ ...filter, field: this.computedRaw(expression) as unknown as string }),
    );
