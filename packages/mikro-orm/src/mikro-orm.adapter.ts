...
export class MikroOrmAdapter implements FilterAdapter {
   ...
   * ORDER BY / WHERE key.
   *
   * A string source may carry a `{alias}` token, substituted at query-build
   ...
   * (`(SELECT COUNT(*) FROM child WHERE child.parent_id = {alias}.id)`).
   *
   * A function source is invoked INSIDE the `raw` callback so it receives the
   * real build-time alias (not a `{alias}` placeholder). It may return a plain
   * SQL string directly, or an adapter-specific value delegated to
   * `computedReturnToSql`.
   *
