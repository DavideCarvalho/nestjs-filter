...
<Aside type="note" title="Zero-config auto-fields (enabled by default)">
...
Configure `maxPageSize` in `FilterModule.forRoot()` to cap the page size (see [Configuration](/nestjs-filter/guides/configuration/)).
...
Query any entity without defining a filter class using `applyDynamic`:

```ts
import { applyDynamic } from '@dudousxd/nestjs-filter';

const qb = em.createQueryBuilder(User);
await applyDynamic(qb, {
  filter: { name: 'Al', status: 'active' },
  sort: ['-createdAt'],
