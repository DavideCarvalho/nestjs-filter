  ...
  <Tab value="MikroORM">
    ...
    @Module({
      imports: [
        ...
        FilterModule.forRoot({ inputNormalizer: 'camelCase', adapter: mikroOrmAdapter }),
  ...
  <Tab value="TypeORM">
    ...
    @Module({
      imports: [
        ...
        FilterModule.forRoot({ inputNormalizer: 'camelCase', adapter: typeOrmAdapter() }),
        FilterModule.forFeature([UserFilter]),
      ],
      controllers: [UsersController],
    })
...
Configure `maxPageSize` in `FilterModule.forRoot()` to cap the page size (see [Module options](#module-options)). Trusted server-side callers (exports, batch jobs) can lift that cap per call with `trustedPageSize` -- see [Trusted page sizes](/docs/guides/repositories#trusted-page-sizes-trustedpagesize).
...
{ filter: { companyId: 'c1' }, distinct: 'status', sort: 'status' }
// → SELECT DISTINCT status FROM ... WHERE company_id = 'c1' ORDER BY status
```
...
// controller
@Post('search')
listStatuses(@ApplyFilter(UserFilter) qb: QueryBuilder<User>) {
  return qb.execute(); // [{ status: 'active' }, { status: 'inactive' }]
}
```
...
// rows: [{ status: 'active' }, { status: 'inactive' }], total: 2
```
...
| `onUnknownKey` | `'ignore' \| 'warn' \| 'throw'` | `'ignore'` | Policy for **structured `filter` keys** that don't match any `@FilterFor` method or auto-field. An unknown column inside a `where[]` clause is not a dispatch key and follows `throwOnInvalid` instead. |
...
| `maxPageSize` | `number` | `100` | Maximum page size for offset pagination. Applies to every call unless the call site passes [`trustedPageSize`](/docs/guides/repositories#trusted-page-sizes-trustedpagesize). |
| `throwOnInvalid` | `boolean` | `false` | When `true`, invalid sorts, distinct fields, and unknown `where[]` columns (at any nesting depth) raise a `BadRequestException` instead of being dropped with a warning. Overridable per-`@Filterable`. |
| `defaultSort` | `string \| SortItem[]` | none | Sort applied when the client sends no `sort`. Overridable per-`@Filterable`, and per-route via `@ApplyFilter`. |
