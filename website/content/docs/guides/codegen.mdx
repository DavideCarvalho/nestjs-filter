...
`nestjsFilterCodegen()` takes no options. On the next codegen run it inspects the discovered routes and emits `filterQuery()` **only** on the leaves whose controller method applies a filter.
