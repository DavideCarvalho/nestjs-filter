...
1. **A SQL string**, optionally with an `{alias}` token (above) — substituted with the query's root alias at build time.
2. **A function** `(ctx) => string`, where `ctx = { alias, em }` (the real build-time alias and the ORM's `EntityManager`/`DataSource`) — for expressions that need runtime context rather than a static string:
...
@Filterable({
  entity: Author,
  computed: {
    booksCount: { source: '(SELECT COUNT(*) FROM books WHERE books.author_id = {alias}.id)', type: 'number' },
  },
})
```
...
Besides the inline `computed` map, a method can be declared computed with `@Computed` — the alias defaults to the method name, and the method body is the source. A `@Computed` method is always wrapped as a **function** source (form 2 or 3 above), never the bare-string form — so the `{alias}` token (form 1, inline-map strings only) is never substituted in a `@Computed` method's return value. Take the `ctx.alias` parameter instead:
...
```
GET /work-orders?sort=-openSubwos
GET /work-orders?openSubwos[gt]=0
```
...
A bare inline entry (`computed: { subwosCount: '...' }` with no `type`) types only the field name in `filterQuery()` — `where('subwosCount', ...)` is accepted, but the value isn't narrowed to a specific type.
