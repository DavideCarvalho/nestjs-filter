...
Besides the inline `computed` map, a method can be declared computed with `@Computed` — the alias defaults to the method name, and the method body is the source, using the exact same three forms above:
