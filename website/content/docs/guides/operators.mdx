...
<Callout type="info">
  Only scalar comparison operators have symbol aliases. Array, range, and unary
...
## Client-side filter builder

`@dudousxd/nestjs-filter-client` exports `FilterQueryBuilder` (and the field-typed `TypedFilterQueryBuilder`, produced by [codegen](/docs/guides/codegen)) for building `ColumnFilter`/structured-input payloads from front-end code. Static call sites use `where(field, operator, value)`, which gives full compile-time overload checking on the typed builder.

For runtime-driven input you don't control at compile time — a DataGrid column filter model, a saved-filter blob, or any other data-driven UI — use the `whereDynamic`/`sortDynamic` escape hatch instead:

```ts
import { filterQueryTyped } from '@dudousxd/nestjs-filter-client';

const query = filterQueryTyped<UserFilterFields>()
  .whereDynamic(column.id, filterModel.operator, filterModel.value)
  .sortDynamic(sortModel.field, sortModel.direction)
  .build();
```

`whereDynamic(field, operator, value?)` and `sortDynamic(field, direction)` accept a runtime `field` string instead of a statically-known key, while still validating `operator` against the known `FilterOperator` list and `value` against the operator's expected shape at call time. Both **replace** any existing condition on the same field, exactly like `where()`/`sort()`. An operator string that isn't a recognized `FilterOperator` throws immediately, rather than silently being treated as an `equals` value — dynamic input is more likely to carry a stale or mistyped operator than a hardcoded call site.

<Callout type="info">
  Prefer `where()`/`sort()` when the field and operator are known statically — they give you full compile-time narrowing on `TypedFilterQueryBuilder` that `whereDynamic`/`sortDynamic` intentionally give up in exchange for accepting arbitrary runtime field names.
</Callout>

### Bulk-applying resolved filters — `fromFilters`

When you already hold an array of resolved `{ field, operator, value }` triples — a saved view, a persisted filter blob, or your own filter-dropdown state — `fromFilters` replays them onto the builder in one call, instead of hand-writing the same loop and `if (field)` guard at every call site:

```ts
import { filterQueryTyped } from '@dudousxd/nestjs-filter-client';

const query = filterQueryTyped<UserFilterFields>()
  .fromFilters(savedView.filters, { skip: currentColumnId })
  .build();
```

Each item funnels through `whereDynamic`, so operator/value validation, unary-value stripping, and replace-per-field semantics all stay centralized there. Items with a falsy `field` are skipped, and `opts.skip` drops the filter for a single column — the "apply every filter **except** the current column's" pattern a self-excluding filter dropdown needs. On the typed builder, `field` (and `opts.skip`) narrow to the route's fields, so a filter array narrowed up to those fields applies with no `as` cast.

There's also a one-shot `filterQueryFromFilters(filters, opts?)` mirroring `tanstackTableToFilterQuery`, equivalent to `filterQuery().fromFilters(filters, opts).build()`.

<Callout type="info">
  `fromFilters` is complementary to `applyTanstackTableState`/`tanstackTableToFilterQuery` (see the [React guide](/docs/guides/react#tanstack-table-interop)): those take vanilla TanStack `{ id, value }` state and *infer* the operator, while `fromFilters` is for when the operator is already resolved in application code.
</Callout>

### Payload types — `UnpagedFilterQuery` and `ColumnFilterClause`

`build()` returns `FilterQueryResult`. For the same query **without** the paging window — a CSV/report export where the server owns the page size, a count-only request, a prefetch, a cache key — annotate it `UnpagedFilterQuery`, the base `FilterQueryResult` extends:

```ts
import type { UnpagedFilterQuery } from '@dudousxd/nestjs-filter-client';

function exportBody(query: UnpagedFilterQuery) {
  // the server decides how much it streams
}
...
<Callout type="warn">
  Do not write `Omit<FilterQueryResult, 'paginate'>` instead. `FilterQueryResult` carries `[key: string]: unknown` so `set()` extras survive, and `Omit` rebuilds a type from `keyof` — which for an index-signature type is only `string | number`. The Omit therefore collapses to `{ [x: string]: unknown }`: every named key erased, every typo accepted, no compile error to warn you.
</Callout>

One entry of `filter.where` is a `ColumnFilterClause` — either a `ColumnFilter` predicate or a `ColumnFilterGroup`, a pure boolean group with no column of its own, which is what the runtime already accepts and what you compose when building groups programmatically rather than through `.or()` / `.and()`:
...
`ColumnFilter` still requires `field` and `operator`: the group is a separate member of the union rather than a loosening, so `{ field: 'status', value: 'active' }` — a predicate whose operator was forgotten — remains a compile error.
...
JSON sub-paths can also be used for **sorting** — see [Sort](/docs/getting-started#sort).
...
- `operator` is one of the 22 known operators (or a [symbol alias](#operator-aliases), which is normalized in place to its canonical form)
