import type { BaseLayoutProps } from 'fumadocs-ui/layouts/shared';
import { appName, gitConfig } from './shared';
import { librarySwitcher } from './libraries';

/**
 * Mono wordmark with the sky "wire" dot — the same identity the landing
 * header uses: a glowing sky dot followed by the package name in a
 * monospace face. Keeps the home → docs transition visually continuous.
 */
function NavTitle() {
  return (
    <span className="inline-flex items-center gap-2 font-mono text-[15px] font-semibold tracking-tight">
      <span
        aria-hidden
        className="size-2 rounded-full bg-sky-400 shadow-[0_0_8px_2px] shadow-sky-500/50"
      />
      {appName}
    </span>
  );
}

export function baseOptions(): BaseLayoutProps {
  return {
    nav: {
      title: <NavTitle />,
    },
    links: [librarySwitcher(appName)],
    githubUrl: `https://github.com/${gitConfig.user}/${gitConfig.repo}`,
  };
}
