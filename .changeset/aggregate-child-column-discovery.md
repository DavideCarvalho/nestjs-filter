...
- Numeric classification stays conservative: a bare `number`, `number | null`/`number | undefined`, or a single-argument `Opt<number>` wrapper qualifies; anything else (including a property with no explicit type annotation) is skipped rather than guessed at, so codegen never over-types a path the server would 400 on.
